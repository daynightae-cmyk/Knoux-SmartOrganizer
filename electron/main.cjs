...
async function downloadOrganizationPlan(context, limit) { const downloads = app.getPath('downloads'); const data = await walk(downloads, context, { limit }); const plans = data.files.filter(file => path.dirname(file.path) === downloads).map(file => ({ source: file.path, category: downloadCategory(file.path), destination: path.join(downloads, downloadCategory(file.path), path.basename(file.path)), size: file.size })); return { downloads, plans, partial: data.partial }; }
